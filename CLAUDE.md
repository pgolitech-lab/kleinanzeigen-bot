...
- **API surface:** `web/api_ma.py` — 20+ endpoint'ов под `/api/ma/*`: pipeline, threads, messages (review payload + lock), send/skip/sold/regenerate/edit-ru/edit-de/instruction, translate-draft, lang-check, compose, autopilot start/stop/preview, suggest-reply, thread wait, settings GET/POST
- **Send-флоу треда (2026-07-19):** три bottom-sheet-этапа по «📨 Отправить» в `thread.js` (правки textarea применяются до входа через edit-ru/edit-de):
  1. **Выбор**: [🌐 Перевести на язык клиента и отправить → `POST /messages/{id}/translate-draft` (перевод в de_answer + RU-зеркало при кириллице, БЕЗ отправки)] [📨 Отправить как есть] [🤖 Предложить текст от ЛЛМ → `/suggest-reply`] [Отменить]
  2. **Подготовка**: превью + RU-зеркало, [Отправить] [✏️ Редактировать → `/edit-de`] [🤖 Переписать идею ЛЛМ… → `/instruction` со свободным текстом оператора] [Отменить]
  3. **Финальный текст**: ровно то, что уйдёт клиенту. **Двухслойный языковой guard**: (а) мгновенная кириллическая эвристика `wrongLangForClient`; (б) `POST /messages/{id}/lang-check` → `claude.detect_text_lang` (Haiku) — до вердикта кнопка «Отправить» выключена. Несовпадение → блок; разблокировка ТОЛЬКО галочкой «Разрешаю отправить текст не на языке клиента» → `/send {mode:as_is, force:true}`. 502 от lang-check НЕ снимает блок эвристики; серверный 409-guard в `/send` (кириллица не-кириллическому клиенту без force) — последняя линия. ИНВАРИАНТ: клиенту никогда не уходит текст не на его языке без явного разрешения оператора.
...
- `modules/parser.py` — Playwright `parse_ad`, regex для URL/ad_id, `clean_email_body`, `is_junk_subject` blacklist, `detect_ad_state` (Gelöscht/Reserviert/active).
  - **`clean_email_body(text, sender_name=None)`** срезает служебный текст Kleinanzeigen. Два пути: (а) web-relay уведомления (клиент ответил через сайт/приложение KZ — приходит HTML-only письмо, `_strip_html` схлопывает его в ОДНУ строку) обрабатывает `_strip_relay_template`: вырезает текст между шапкой «…gesendet/erhalten. Nachricht/Antwort von <имя>» и футером «Beantworte diese Nachricht…» (обязателен как признак шаблона), затем точно срезает имя отправителя (`sender_name` = `buyer_display_name`, через `re.escape` — напр. `B.ST`); (б) иначе — старые построчные `_NOISE_HEADER/FOOTER_PATTERNS`. `incoming._process_incoming` прокидывает имя в `clean_email_body`.
  - **Self-heal** `incoming.reclean_incoming_bodies()` + `count_polluted_incoming()`: находит по SQL-маркерам футера входящие, где остался relay-шаблон (письма до фикса 2026-07-31 или регресс формата), перечищает de_client + пере-переводит ru_client (Haiku). Job `reclean_bodies` в scheduler раз в 30 мин + разово при `start()`. `count_polluted_incoming` отдаётся в `/health` полем `polluted_incoming` (0 при норме; >0 стабильно = регресс → `stuck` логируется ERROR → monitor_errors алертит). **Прямую запись в БД из внешнего Bash блокирует auto-mode classifier — чистит только сам процесс бота.**
- `modules/claude.py` — Anthropic SDK обёртки:
  ...
  - `detect_text_lang(text)` (Haiku, json_schema) — язык финального текста для lang-check в send-флоу MA; `'und'` = языково-нейтральный текст (числа, «Ok», emoji), не блокирует
...
- **json_schema в `output_config` требует явный `additionalProperties: false`** для object-типов — иначе 400 на живом вызове API. В unit-тестах claude замокан, ошибка ловится только e2e (так поймали баг в `_LANG_DETECT_SCHEMA` 2026-07-19). Проверять при добавлении каждой новой схемы.
